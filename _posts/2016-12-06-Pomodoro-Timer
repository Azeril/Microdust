...
这个就不详细说了，主要讲一下怎么使用angularJS来实现这个倒计时的功能。
...
4、然后是实现倒计时函数（updateTimer()），因为秒数是递减的，所以seconds每次都减1，当秒数小于0了就要切换倒计时类型了，背景填充颜色也要随之切换。填充百分率即当前流失时间占原始时间的百分比，即[（originTime - timeLeft）/originTime ]*100% => (1 - seconds/ori) * 100%。具体代码如下：
